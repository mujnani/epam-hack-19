...
function Bubbles(container, options) {
  ...
  let animationTime = options.animationTime || 200 // how long it takes to animate chat bubble, also set in CSS
  let typeSpeed = options.typeSpeed || 5 // delay per character, to simulate the machine "typing"
  let widerBy = options.widerBy || 2 // add a little extra width to bubbles to make sure they don't break
  let sidePadding = options.sidePadding || 6 // padding on both sides of chat bubbles
  let recallInteractions = options.recallInteractions || 0 // number of interactions to be remembered and brought back upon restart
  let inputCallbackFn = options.inputCallbackFn || false // should we display an input field?

  let standingAnswer = options.standingAnswer || "ice" // remember where to restart convo if interrupted
  ...
  this.reply = function(turn, lastAnswer) {
    iceBreaker = typeof turn === "undefined"
    turn = !iceBreaker ? turn : _convo[standingAnswer]
    questionsHTML = ""
    if (!turn) return
      ...
      for (var i = 0; i < turn.reply.length; i++) {
        ;(function(el, count) {
          questionsHTML += `
            <span
              class="bubble-button"
              style="animation-delay: ${animationTime / 2 * count}ms"
    ...
    let _says = turn.says;
    if (turn.replace && turn.replace.length) {
      _says = _says.map(item => turn.replace.map(rep => item.replace(`{${rep}}`, lastAnswer || '')))
    }
    orderBubbles(_says, function() {
  ...
  var addBubble = function(say, posted, reply, live) {
    ...
    var avatar = `<div class="avatar"><img src="${options.serviceAvatar}"></div>`;
    ...
    if (reply !== "") {
      (bubble.querySelectorAll('.avatar img'))[0].setAttribute('src', options.customerAvatar)
      ...
      bubble.addEventListener("click", function(e) {
        ...
        const _answer  = _target.getAttribute('data-answer');
        const _question  = _target.getAttribute('data-question');
        _target.classList.add('bubble-pick');
        if (!this.classList.contains("bubble-picked")) {
          _this.answer(_answer, _question);
        }
